...
router.get('/FINAL/CHECK-HPTHI005', async (req, res) => {

  return res.json(HPTHI005db['PO']);
});


router.post('/FINAL/HPTHI005db', async (req, res) => {
  //-------------------------------------
  // console.log('--HPTHI005db--');
...
router.post('/FINAL/GETINtoHPTHI005', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHPTHI005--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/FINAL/HPTHI005-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI005-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < HPTHI005db['ItemPickcode'].length; i++) {
    if (HPTHI005db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = HPTHI005db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HPTHI005db['inspectionItem'] = ITEMSS;
    HPTHI005db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HPTHI005db["PO"], 'CP': HPTHI005db["CP"], 'ITEMs': HPTHI005db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": HPTHI005db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HPTHI005db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HPTHI005db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HPTHI005db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HPTHI005db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              HPTHI005db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          HPTHI005db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HPTHI005db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          HPTHI005db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          HPTHI005db["SPEC"] = '';
          if (findcp[0]['FINAL'][i]['SPECIFICATIONve'] !== undefined) {
            if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'BTW') {
              HPTHI005db["SPEC"] = `${findcp[0]['FINAL'][i]['SPECIFICATIONve']['BTW_LOW']}-${findcp[0]['FINAL'][i]['SPECIFICATIONve']['BTW_HI']}`;
            } else if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'HIM(>)') {
              HPTHI005db["SPEC"] = `>${findcp[0]['FINAL'][i]['SPECIFICATIONve']['HIM_L']}`;
            } else if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'LOL(<)') {
              HPTHI005db["SPEC"] = `<${findcp[0]['FINAL'][i]['SPECIFICATIONve']['LOL_H']}`;
            } else if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'Actual') {
              HPTHI005db["SPEC"] = 'Actual';
            }
          }
          ...
          let date = Date.now()
          let REFLOT = await mongodb.find(PATTERN, "referdata", { "MATCP": HPTHI005db['MATCP'], "ITEMS": ITEMSS, "EXP": { $gt: date } });

          console.log(REFLOT)

          if (REFLOT.length > 0) {
            HPTHI005db["REFLOT"] = REFLOT[0]['TPKLOT'];
          }

          HPTHI005db["FREQUENCY"] = findcp[0]['FINAL'][i]['FREQUENCY'];



          HPTHI005db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:16175/FINAL/HPTHI005-feedback',
              { json: { "PO": HPTHI005db['PO'], "ITEMs": HPTHI005db['inspectionItem'] } },
...
router.post('/FINAL/HPTHI005-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI005-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HPTHI005db['GAPnameListdata'] = graph[0];//confirmdata
    HPTHI005db['GAP'] = HPTHI005db['GAPnameListdata'][`GT${HPTHI005db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/FINAL/HPTHI005-preview', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI005-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/FINAL/HPTHI005-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI005-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HPTHI005db['preview'][0]

    if (HPTHI005db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HPTHI005db['preview'][0]

      pushdata['V5'] = HPTHI005db['GAP'];
      pushdata['V1'] = `${HPTHI005db['confirmdata'].length + 1}:${pushdata['V1']}`;

      if (HPTHI005db['GAP'] != '') {

        HPTHI005db['confirmdata'].push(pushdata);
        HPTHI005db['preview'] = [];
        output = 'OK';
        HPTHI005db['GAP'] = HPTHI005db['GAPnameListdata'][`GT${HPTHI005db['confirmdata'].length + 1}`]
      } else {
    ...
    } else if (HPTHI005db['RESULTFORMAT'] === 'Number') {

      let pushdata = HPTHI005db['preview'][0]

      pushdata['V5'] = HPTHI005db['confirmdata'].length + 1
      pushdata['V1'] = `${HPTHI005db['confirmdata'].length + 1}:${pushdata['V1']}`

      HPTHI005db['confirmdata'].push(pushdata);
      HPTHI005db['preview'] = [];
      output = 'OK';
    } else if (HPTHI005db['RESULTFORMAT'] === 'CAL2') {
...
router.post('/FINAL/HPTHI005-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI005-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      HPTHI005db["PCSleft"] = `${parseInt(HPTHI005db["PCS"]) - oblist.length}`;
      if (HPTHI005db['RESULTFORMAT'] === 'Number' || HPTHI005db['RESULTFORMAT'] === 'Text' || HPTHI005db['RESULTFORMAT'] === 'Graph') {
        ...
        HPTHI005db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HPTHI005db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(HPTHI005db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        for (i = 0; i < feedback[0]['CHECKlist'].length; i++) {
          if (input["ITEMs"] === feedback[0]['CHECKlist'][i]['key']) {
            ...
            if (HPTHI005db['FREQUENCY'] === 'time/D'|| HPTHI005db['FREQUENCY'] === 'time/6M'||HPTHI005db['FREQUENCY'] === 'pcs/M'||HPTHI005db['FREQUENCY'] === 'time/Year'||HPTHI005db['FREQUENCY'] === 'pcs/Y') {
              let resp = await axios.post('http://127.0.0.1:16175/FINAL/REFLOTSET', {
                "PO": HPTHI005db['PO'],
                "MATCP": HPTHI005db['CP'],
                "FREQUENCY": HPTHI005db['FREQUENCY'],
                "ITEMs": HPTHI005db['inspectionItem'],
                "TPKLOT": HPTHI005db['TPKLOT'],
                "INS": HPTHI005db['INS']
              });
            }
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HPTHI005db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (HPTHI005db['INTERSEC'] !== '') {
                core = parseFloat(HPTHI005db['INTERSEC'])
              } else {
              ...
              try {
                ...
                HPTHI005db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (HPTHI005db['GRAPHTYPE'] == 'CDT') {
              ...
              //
            } else if (HPTHI005db['GRAPHTYPE'] == 'CDT(S)') {
      ...
      HPTHI005db["ITEMleftUNIT"] = '';
      HPTHI005db["ITEMleftVALUE"] = '';
    }
...
router.post('/FINAL/HPTHI005-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI005fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/FINAL/HPTHI005-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI005fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/FINAL/HPTHI005-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI005fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HPTHI005db['confirmdata'].length
    if (all > 0) {
      HPTHI005db['confirmdata'].pop();
    }
...
router.post('/FINAL/HPTHI005-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI005-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (HPTHI005db['RESULTFORMAT'] === 'Number' || HPTHI005db['RESULTFORMAT'] === 'Text') {

    HPTHI005db["value"] = [];
    for (i = 0; i < HPTHI005db['confirmdata'].length; i++) {
      HPTHI005db["value"].push({
        "PO1": HPTHI005db["inspectionItemNAME"],
        "PO2": HPTHI005db['confirmdata'][i]['V1'],
        "PO3": HPTHI005db['confirmdata'][i]['V2'],
        "PO4": HPTHI005db['confirmdata'][i]['V3'],
        "PO5": HPTHI005db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HPTHI005db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < HPTHI005db["value"].length; i++) {
        mean01.push(parseFloat(HPTHI005db["value"][i]["PO3"]));
        mean02.push(parseFloat(HPTHI005db["value"][i]["PO5"]));
      }
      ...
      HPTHI005db["value"].push({
        "PO1": 'Mean',
        "PO2": HPTHI005db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HPTHI005db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HPTHI005db['RESULTFORMAT'] === 'OCR' || HPTHI005db['RESULTFORMAT'] === 'Picture') {

  } else if (HPTHI005db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < HPTHI005db['confirmdata'].length; i++) {
      HPTHI005db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HPTHI005db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HPTHI005db['RESULTFORMAT'] === 'Number' ||
    HPTHI005db['RESULTFORMAT'] === 'Text' ||
    HPTHI005db['RESULTFORMAT'] === 'OCR' ||
    HPTHI005db['RESULTFORMAT'] === 'Picture' || HPTHI005db['RESULTFORMAT'] === 'Graph') {
    request.post(
  ...
  //-------------------------------------
  return res.json(HPTHI005db);
});

router.post('/FINAL/HPTHI005-REFLOT', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI005-REFLOT--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
