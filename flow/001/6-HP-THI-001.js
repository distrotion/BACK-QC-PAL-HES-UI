...
router.get('/FINAL/CHECK-HPTHI001', async (req, res) => {

  return res.json(HPTHI001db['PO']);
});


router.post('/FINAL/HPTHI001db', async (req, res) => {
  //-------------------------------------
  // console.log('--HPTHI001db--');
...
router.post('/FINAL/GETINtoHPTHI001', async (req, res) => {
  //-------------------------------------
  console.log('--GETINtoHPTHI001--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/FINAL/HPTHI001-geteachITEM', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI001-geteachITEM--');
  console.log(req.body);
  let inputB = req.body;
  ...
  for (i = 0; i < HPTHI001db['ItemPickcode'].length; i++) {
    if (HPTHI001db['ItemPickcode'][i]['value'] === inputB['ITEMs']) {
      ITEMSS = HPTHI001db['ItemPickcode'][i]['key'];
    }
  }
  ...
  if (ITEMSS !== '') {
    ...
    //-------------------------------------
    HPTHI001db['inspectionItem'] = ITEMSS;
    HPTHI001db['inspectionItemNAME'] = inputB['ITEMs'];
    let input = { 'PO': HPTHI001db["PO"], 'CP': HPTHI001db["CP"], 'ITEMs': HPTHI001db['inspectionItem'] };
    //-------------------------------------
    if (input['PO'] !== undefined && input['CP'] !== undefined && input['ITEMs'] !== undefined) {
      let findcp = await mongodb.find(PATTERN, PATTERN_01, { "CP": input['CP'] });
      let UNITdata = await mongodb.find(master_FN, UNIT, {});
      let masterITEMs = await mongodb.find(master_FN, ITEMs, { "masterID": HPTHI001db['inspectionItem'] });
      ...
      for (i = 0; i < findcp[0]['FINAL'].length; i++) {
        if (findcp[0]['FINAL'][i]['ITEMs'] === input['ITEMs']) {
          ...
          if (masterITEMs.length > 0) {
            //
            HPTHI001db["RESULTFORMAT"] = masterITEMs[0]['RESULTFORMAT']
            HPTHI001db["GRAPHTYPE"] = masterITEMs[0]['GRAPHTYPE']
            //------------------------------------

            let graph = await mongodb.find(PATTERN, GRAPH_TABLE, {});
            HPTHI001db['GAPnameList'] = [];
            for (k = 0; k < graph.length; k++) {
              HPTHI001db['GAPnameList'].push(graph[k]['NO']);
            }
          }

          for (j = 0; j < UNITdata.length; j++) {
            if (findcp[0]['FINAL'][i]['UNIT'] == UNITdata[j]['masterID']) {
              HPTHI001db["UNIT"] = UNITdata[j]['UNIT'];
            }
          }

          console.log(findcp[0]['FINAL'][i]['POINT']);

          HPTHI001db["POINTs"] = findcp[0]['FINAL'][i]['POINT'];
          HPTHI001db["PCS"] = findcp[0]['FINAL'][i]['PCS'];
          HPTHI001db["PCSleft"] = findcp[0]['FINAL'][i]['PCS'];

          HPTHI001db["SPEC"] = '';
          if (findcp[0]['FINAL'][i]['SPECIFICATIONve'] !== undefined) {
            if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'BTW') {
              HPTHI001db["SPEC"] = `${findcp[0]['FINAL'][i]['SPECIFICATIONve']['BTW_LOW']}-${findcp[0]['FINAL'][i]['SPECIFICATIONve']['BTW_HI']}`;
            } else if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'HIM(>)') {
              HPTHI001db["SPEC"] = `>${findcp[0]['FINAL'][i]['SPECIFICATIONve']['HIM_L']}`;
            } else if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'LOL(<)') {
              HPTHI001db["SPEC"] = `<${findcp[0]['FINAL'][i]['SPECIFICATIONve']['LOL_H']}`;
            } else if (findcp[0]['FINAL'][i]['SPECIFICATIONve']['condition'] === 'Actual') {
              HPTHI001db["SPEC"] = 'Actual';
            }
          }
          ...
          let date = Date.now()
          let REFLOT = await mongodb.find(PATTERN, "referdata", { "MATCP": HPTHI001db['MATCP'], "ITEMS": ITEMSS, "EXP": { $gt: date } });

          console.log(REFLOT)

          if (REFLOT.length > 0) {
            HPTHI001db["REFLOT"] = REFLOT[0]['TPKLOT'];
          }

          HPTHI001db["FREQUENCY"] = findcp[0]['FINAL'][i]['FREQUENCY'];



          HPTHI001db["INTERSEC"] = masterITEMs[0]['INTERSECTION'];
          output = 'OK';
          let findpo = await mongodb.find(MAIN_DATA, MAIN, { "PO": input['PO'] });
          if (findpo.length > 0) {
            request.post(
              'http://127.0.0.1:16175/FINAL/HPTHI001-feedback',
              { json: { "PO": HPTHI001db['PO'], "ITEMs": HPTHI001db['inspectionItem'] } },
...
router.post('/FINAL/HPTHI001-geteachGRAPH', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI001-geteachGRAPH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  try {
    ...
    console.log(graph);
    HPTHI001db['GAPnameListdata'] = graph[0];//confirmdata
    HPTHI001db['GAP'] = HPTHI001db['GAPnameListdata'][`GT${HPTHI001db['confirmdata'].length + 1}`]
  }
  catch (err) {
...
router.post('/FINAL/HPTHI001-preview', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI001-preview--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/FINAL/HPTHI001-confirmdata', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI001-confirmdata--');
  console.log(req.body);
  // let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  //-------------------------------------
  try {
    let datapush = HPTHI001db['preview'][0]

    if (HPTHI001db['RESULTFORMAT'] === 'Graph') {
      let pushdata = HPTHI001db['preview'][0]

      pushdata['V5'] = HPTHI001db['GAP'];
      pushdata['V1'] = `${HPTHI001db['confirmdata'].length + 1}:${pushdata['V1']}`;

      if (HPTHI001db['GAP'] != '') {

        HPTHI001db['confirmdata'].push(pushdata);
        HPTHI001db['preview'] = [];
        output = 'OK';
        HPTHI001db['GAP'] = HPTHI001db['GAPnameListdata'][`GT${HPTHI001db['confirmdata'].length + 1}`]
      } else {
    ...
    } else if (HPTHI001db['RESULTFORMAT'] === 'Number') {

      let pushdata = HPTHI001db['preview'][0]

      pushdata['V5'] = HPTHI001db['confirmdata'].length + 1
      pushdata['V1'] = `${HPTHI001db['confirmdata'].length + 1}:${pushdata['V1']}`

      HPTHI001db['confirmdata'].push(pushdata);
      HPTHI001db['preview'] = [];
      output = 'OK';
    } else if (HPTHI001db['RESULTFORMAT'] === 'CAL2') {
...
router.post('/FINAL/HPTHI001-feedback', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI001-feedback--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
  ...
  if (input["PO"] !== undefined && input["ITEMs"] !== undefined) {
    ...
    if (feedback.length > 0 && feedback[0]['FINAL'] != undefined && feedback[0]['FINAL'][NAME_INS] != undefined && feedback[0]['FINAL'][NAME_INS][input["ITEMs"]] != undefined) {
      ...
      HPTHI001db["PCSleft"] = `${parseInt(HPTHI001db["PCS"]) - oblist.length}`;
      if (HPTHI001db['RESULTFORMAT'] === 'Number' || HPTHI001db['RESULTFORMAT'] === 'Text' || HPTHI001db['RESULTFORMAT'] === 'Graph') {
        ...
        HPTHI001db["ITEMleftUNIT"] = [{ "V1": "FINAL", "V2": `${oblist.length}` }];
        HPTHI001db["ITEMleftVALUE"] = ITEMleftVALUEout;

      } else {

      }
      // output = 'OK';
      if ((parseInt(HPTHI001db["PCS"]) - oblist.length) == 0) {
        //CHECKlist
        for (i = 0; i < feedback[0]['CHECKlist'].length; i++) {
          if (input["ITEMs"] === feedback[0]['CHECKlist'][i]['key']) {
            ...
            if (HPTHI001db['FREQUENCY'] === 'time/D'|| HPTHI001db['FREQUENCY'] === 'time/6M'||HPTHI001db['FREQUENCY'] === 'pcs/M'||HPTHI001db['FREQUENCY'] === 'time/Year'||HPTHI001db['FREQUENCY'] === 'pcs/Y') {
              let resp = await axios.post('http://127.0.0.1:16175/FINAL/REFLOTSET', {
                "PO": HPTHI001db['PO'],
                "MATCP": HPTHI001db['CP'],
                "FREQUENCY": HPTHI001db['FREQUENCY'],
                "ITEMs": HPTHI001db['inspectionItem'],
                "TPKLOT": HPTHI001db['TPKLOT'],
                "INS": HPTHI001db['INS']
              });
            }
        ...
        if (masterITEMs.length > 0) {
          ...
          if (masterITEMs[0]['RESULTFORMAT'] === 'Number') {
            ...
            if (HPTHI001db['GRAPHTYPE'] == 'CDE') {
              ...
              let core = 0;
              if (HPTHI001db['INTERSEC'] !== '') {
                core = parseFloat(HPTHI001db['INTERSEC'])
              } else {
              ...
              try {
                ...
                HPTHI001db[`INTERSEC_ERR`] = 1;
              }

              //
            } else if (HPTHI001db['GRAPHTYPE'] == 'CDT') {
              ...
              //
            } else if (HPTHI001db['GRAPHTYPE'] == 'CDT(S)') {
      ...
      HPTHI001db["ITEMleftUNIT"] = '';
      HPTHI001db["ITEMleftVALUE"] = '';
    }
...
router.post('/FINAL/HPTHI001-SETZERO', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI001fromINS--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
...
router.post('/FINAL/HPTHI001-CLEAR', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI001fromINS--');
  console.log(req.body);
  let input = req.body;
...
router.post('/FINAL/HPTHI001-RESETVALUE', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI001fromINS--');
  console.log(req.body);
  let input = req.body;
  ...
  try {
    ...
    let all = HPTHI001db['confirmdata'].length
    if (all > 0) {
      HPTHI001db['confirmdata'].pop();
    }
...
router.post('/FINAL/HPTHI001-FINISH', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI001-FINISH--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'OK';

  if (HPTHI001db['RESULTFORMAT'] === 'Number' || HPTHI001db['RESULTFORMAT'] === 'Text') {

    HPTHI001db["value"] = [];
    for (i = 0; i < HPTHI001db['confirmdata'].length; i++) {
      HPTHI001db["value"].push({
        "PO1": HPTHI001db["inspectionItemNAME"],
        "PO2": HPTHI001db['confirmdata'][i]['V1'],
        "PO3": HPTHI001db['confirmdata'][i]['V2'],
        "PO4": HPTHI001db['confirmdata'][i]['V3'],
        "PO5": HPTHI001db['confirmdata'][i]['V4'],
        "PO6": "-",
        "PO7": "-",
        "PO8": '-',
        "PO9": i + 1,
    ...
    if (HPTHI001db["value"].length > 0) {
      let mean01 = [];
      let mean02 = [];
      for (i = 0; i < HPTHI001db["value"].length; i++) {
        mean01.push(parseFloat(HPTHI001db["value"][i]["PO3"]));
        mean02.push(parseFloat(HPTHI001db["value"][i]["PO5"]));
      }
      ...
      HPTHI001db["value"].push({
        "PO1": 'Mean',
        "PO2": HPTHI001db['confirmdata'][0]['V1'],
        "PO3": avg1,
        "PO4": HPTHI001db['confirmdata'][0]['V3'],
        "PO5": avg2,
      });
    }

  } else if (HPTHI001db['RESULTFORMAT'] === 'OCR' || HPTHI001db['RESULTFORMAT'] === 'Picture') {

  } else if (HPTHI001db['RESULTFORMAT'] === 'Graph') {
    ...
    for (i = 0; i < HPTHI001db['confirmdata'].length; i++) {
      HPTHI001db["value"].push({
        ...
        "PO6": "-",
        "PO7": "-",
        "PO8": HPTHI001db['confirmdata'][i]['V5'],
        "PO9": i + 1,
        "PO10": "AUTO",
      });
    }
  ...
  if (HPTHI001db['RESULTFORMAT'] === 'Number' ||
    HPTHI001db['RESULTFORMAT'] === 'Text' ||
    HPTHI001db['RESULTFORMAT'] === 'OCR' ||
    HPTHI001db['RESULTFORMAT'] === 'Picture' || HPTHI001db['RESULTFORMAT'] === 'Graph') {
    request.post(
  ...
  //-------------------------------------
  return res.json(HPTHI001db);
});

router.post('/FINAL/HPTHI001-REFLOT', async (req, res) => {
  //-------------------------------------
  console.log('--HPTHI001-REFLOT--');
  console.log(req.body);
  let input = req.body;
  //-------------------------------------
  let output = 'NOK';
